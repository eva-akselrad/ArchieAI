    ...
    if session_id.is_some() {
        ...
        match tokio::fs::read_to_string("src/templates/home.html").await {
            ...
            Err(e) => (
                ...
                format!("Failed to load home page Error {e}"),
            )
                .into_response(),
        }
    }
...
async fn api_archie(
    ...
    let history: Option<Vec<archie_ai_rust::gem_interface::Message>> =
    ...
    let answer = match state
        .ai_interface
        .archie_streaming(question.clone(), history)
        .await
    {
    ...
    if let Some(ref sid) = session_id {
        state
            .session_manager
            .add_message(sid, "user".to_string(), question.clone());
        state
            .session_manager
            .add_message(sid, "assistant".to_string(), answer.clone());
    }
 ...
 */
async fn get_session_history(State(state): State<AppState>, headers: HeaderMap) -> Response {
    let session_id = get_cookie(&headers, "session_id");
 ...
 */
async fn list_user_sessions(State(state): State<AppState>, headers: HeaderMap) -> Response {
    let user_email = get_cookie(&headers, "user_email");
    ...
    match user_email {
        Some(email) => {
            let sessions = state
                .session_manager
                .get_all_user_sessions_with_preview(&email);
    ...
    match state.session_manager.get_session(&session_id) {
        Some(session_data) => {
            ...
            if session_data.user_email != user_email
                && Some(session_id.clone()) != current_session_id
            {
                return (
 ...
 */
async fn create_new_session(State(state): State<AppState>, headers: HeaderMap) -> Response {
    let user_email = get_cookie(&headers, "user_email");
    ...
    if let Ok(headers) = response.headers_mut().try_insert(
        header::SET_COOKIE,
        create_cookie_header("session_id", &session_id)
            .parse()
            .unwrap(),
    ) {
        let _ = headers;
    ...
    match state.session_manager.get_session(&session_id) {
        Some(session_data) => {
            ...
            if let Ok(headers) = response
                .headers_mut()
                .try_insert(header::SET_COOKIE, cookie_header.parse().unwrap())
            {
                let _ = headers;
            }
...
async fn chats_post(
        ...
        return match tokio::fs::read_to_string("src/templates/home.html").await {
            Ok(mut content) => {
                ...
                content = content.replace(
                    "</body>",
                    "<script>alert('Please provide a valid email address');</script></body>",
                );
    ...
    if password.is_empty() {
        return match tokio::fs::read_to_string("src/templates/home.html").await {
            Ok(mut content) => {
                content = content.replace(
                    "</body>",
                    "<script>alert('Password is required');</script></body>",
                ); //The alerts do not work, like at all. No i will not fix it.(yet) this goes for the other alerts too and the python version as well.
                Html(content).into_response()
            }
            Err(_) => (StatusCode::INTERNAL_SERVER_ERROR, "Failed to load page").into_response(),
        };
    ...
    if state.session_manager.authenticate_user(email, &password) {
        ...
        let session_id = state
            .session_manager
            .create_session(Some(email.to_string()));
        ...
        if state
            .session_manager
            .create_user(email.to_string(), password, ip_address, device_info)
        {
            let session_id = state
                .session_manager
                .create_session(Some(email.to_string()));
            ...
            match tokio::fs::read_to_string("src/templates/home.html").await {
                Ok(mut content) => {
                    content = content.replace(
                        "</body>",
                        "<script>alert('Failed to create account');</script></body>",
                    );
                    Html(content).into_response()
                }
                Err(_) => {
...
mod tests {
    ...
    fn test_get_cookie() {
        ...
        headers.insert(
            header::COOKIE,
            "session_id=test123; user_email=test@example.com"
                .parse()
                .unwrap(),
        );
